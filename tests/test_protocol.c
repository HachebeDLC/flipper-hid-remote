...
int main() {
  ...
  int kb_result = FlipperProtocolParse(kb_packet, sizeof(kb_packet));
  ...
  int mouse_result = FlipperProtocolParse(mouse_packet, sizeof(mouse_packet));
  assert(mouse_result == 0); // 0 indicates success
  
  // Test parsing an invalid packet type
  uint8_t invalid_packet[] = {0x03, 0x00};
  int invalid_result = FlipperProtocolParse(invalid_packet, sizeof(invalid_packet));
